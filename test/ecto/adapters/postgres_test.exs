...
defmodule Ecto.Adapters.PostgresTest do
  ...
  defp normalize(query, operation \\ :all, counter \\ 0) do
    {query, _params, _key} =
      Ecto.Query.Planner.prepare(query, operation, Ecto.Adapters.Postgres, counter)
  ...
  test "delete all with returning" do
    ...
    assert delete_all(query) ==
             ~s{DELETE FROM "schema" AS s0 RETURNING s0."id", s0."x", s0."y", s0."z", s0."w"}
  end
  ...
  test "insert with on conflict" do
    ...
    query =
      insert(
        ...
        [:x, :y],
        [[:x, :y]],
        {:replace_all, [], {:unsafe_fragment, "(\"id\")"}},
        []
      )
